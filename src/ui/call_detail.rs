...
pub fn render(f: &mut Frame, area: Rect, snap: &Snapshot, app: &mut App) {
    ...
    // Fixed left-right split. Left column: message flow (4/5) with
    // diagnostics below (1/5). Right column: raw message (2/3) with the
    // network/media stats below (1/3).
    let cols = Layout::horizontal([Constraint::Percentage(45), Constraint::Percentage(55)])
        .split(chunks[2]);
    let left = Layout::vertical([Constraint::Ratio(4, 5), Constraint::Ratio(1, 5)])
        .split(cols[0]);
    render_flow(f, left[0], &focus.messages, app);
    render_diag(f, left[1], focus);

    let right = Layout::vertical([Constraint::Ratio(2, 3), Constraint::Ratio(1, 3)])
        .split(cols[1]);
    render_raw(f, right[0], &focus.messages, app);
    render_network(f, right[1], focus);
}
